function plotParamSpace_gamma(modelFit, numParam, modNum, sv, bicMatrix, pEst, modName, minMax)
revCM = 64:-1:1;
cMap = jet;
cMap = cMap(revCM,:);
if numParam == 2 % If the model only has two parameters
    ...
    [~, idx] = min(bestFit);
    imagesc(squeeze(modelFit(:,iFit,:)))
    title([modName ' | Optimal \gamma: ' num2str(sv(idx))])
end
colormap(cMap)
...
for iScat = 1:length(bicMatrix)
    if numParam == 3 % Three parameter models (use alpha and gamma)
        if sum(winMod == iScat) % If it is the winning model
            scatter(pEst(modNum).model{1,iScat}(3)*(100/8), 100-(pEst(modNum).model{1,iScat}(1)*100), 'filled','d','MarkerFaceColor',[.5 .5 .5])
        else
            scatter(pEst(modNum).model{1,iScat}(3)*(100/8), 100-(pEst(modNum).model{1,iScat}(1)*100), 'filled','MarkerFaceColor',[.9 .9 .9])
        end
